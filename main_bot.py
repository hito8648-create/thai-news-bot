...
12件程度のニュース見出しから、日本の読者や在住者が最も関心を持つ「最高の一記事」を1件だけ厳選し、投稿文を作成してください。
...
def post_to_threads(text):
    ...
    if res_c.status_code != 200: return False
    c_id = res_c.json().get('id')
    res_p = requests.post(f"https://graph.threads.net/v1.0/{THREADS_USER_ID}/threads_publish", data={
        "creation_id": c_id, "access_token": THREADS_ACCESS_TOKEN
    })
